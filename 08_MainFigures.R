...
# Calculate mean and 95% bootstrap interval
...
LATR_flow_pred <- data.frame(
  weighted.dens = rep(seq(min(LATR_flow_dat$weighted.dens), max(LATR_flow_dat$weighted.dens), length.out = 20),
                      times = n_cuts_size),
...
LATR_fruit_pred <- data.frame(
  weighted.dens = rep(seq(min(LATR_fruits_dat$weighted.dens), max(LATR_fruits_dat$weighted.dens), length.out = 20),
                      times = n_cuts_size),
...
LATR_surv_nat_pred$pred <- predict.gam(LATR_surv_best,newdata = LATR_surv_nat_pred,
                                       exclude = "s(unique.transect)")
...
LATR_surv_exp_pred <- data.frame(
  ...
  unique.transect = "1.FPS",
  transplant = TRUE)
LATR_surv_exp_pred$pred <- predict.gam(LATR_surv_best, newdata = LATR_surv_exp_pred,
                                       exclude = "s(unique.transect)")
...
LATR_recruitment_line$pred <- predict.gam(LATR_recruit_best, newdata = LATR_recruitment_line,
                                          exclude = "s(unique.transect)")
