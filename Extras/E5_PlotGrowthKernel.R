...
# Problem here is that growth has a continuous covariate
# To visualise this, discretise 4 quantiles of the weighted density distribution
...
levelplot(CYIM_lmer_best_kernel, row.values = size_dum, column.values = size_dum, cuts = 30,
          ...
          main = "Gaussian, non-constant variance",
          panel = function(...) {
            panel.levelplot(...)
            grid.points(log(CYIM$vol_t), log(CYIM$vol_t1), pch = ".",
                        gp = gpar(cex = 3, col = alpha("black", 0.5)))})
