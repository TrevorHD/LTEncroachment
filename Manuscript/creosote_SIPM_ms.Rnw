...
Volume was calculated as that of an \tom{elliptic}{I checked the code and actually this is not what we did.} cone: $V_{i} = \pi lwh/3$ where $l$, $w$, and $h$ are the maximum length, maximum width, and height, respectively.
...
Because we had additional, finer-grained cover data for the transplant experiment that we did not have for the observational census, we conducted an additional stand-alone analysis of transplant that explored the influence of covariates at multiple spatial scales (Appendix).
