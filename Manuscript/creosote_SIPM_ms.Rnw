...
\begin{figure}[H]
  ...
  \caption{Example of an ecotone transect at Sevilleta LTER, spanning gradients of creosotebish and black grama grass.}
  \label{fig:waves}
  \end{center}
\end{figure}
...
To avoid over-fitting, we set the `gamma' argument of gam() to 1.5, which increases the complexity penalty, results in smoother fits \citep{Wood2017}, and makes our approach more conservative (other gamma values yielded qualitatively similar results).
...
We verified that the sgt model described the data well by simuating data from it and comparing simulated and real data. 
...
The size- and density-dependent statistical models comprised the sub-models of a non-spatial Integral Projection Model that we used to evaluate how the shrub population growth rate responded to conspecific density, before considering the additional role of dispersal. 
...
\begin{linenomath*} 
\begin{equation} 
K(x\prime,x,\tilde{N}) = S(x,\tilde{N})G(x\prime,x,\tilde{N}) + P(x,\tilde{N})F(x,\tilde{N})R(\tilde{N})C(x\prime)
\end{equation} 
\end{linenomath*}
In the statistical modeling, we explored density ($\tilde{N}$) effects in all of these vital rates except the recruit size distribution.
