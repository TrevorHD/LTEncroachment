...
The process of encroachment generally involves increases in the number or density of woody plants in both time and space \citep{van2000shrub}, which can drive shifts in plant community competition and alter ecosystem processes \citep{schlesinger1990biological, ravi2009can,schlesinger1998plant, knapp2008shrub}.
...
Second, colonization events are driven by the spatial dispersal of propagules, which is commonly summarized as a probability distribution of dispersal distance (``dispersal kernel'').
...
The nature of conspecific density dependence is a critical feature of expansion dynamics but is rarely studied in the context of woody plant encroachment.  
Expansion waves typically correspond to gradients of conspecific density -- high in the back and low at the front -- and demographic rates may be sensitive to density due to intraspecific interactions.
If the demographic effects of density are strictly negative due to competitive effects then fitness is maximized as density goes to zero, at the leading edge of the wave. 
...
Such Allee effects (in the language of population biology) cause demographic rates to be maximized at higher densities behind the leading edge, which ``push'' the expansion forward, leading to qualitatively different dynamics \citep{kot1996dispersal, taylor2005allee, sullivan2017density, lewis1993allee, veit1996dispersal, keitt2001allee}.
Pushed expansion waves generally have different shapes (steeper density gradients) and slower speeds than pulled waves (cite), and require different strategies for managing or decelerating expansion (cite). 
The potential for positive feedbacks is well documented in woody encroachment systems but it remains unclear whether and how strongly these feedbacks decelerate shrub expansion.
Put another way, despite decades of important work on the topic, we still do not know whether expansion waves of woody encroachment are pushed or pulled. 
