...
Strong observational and experimental evidence for strictly negative density dependence, in all vital rates and at all sizes, was surprising given widespread evidence for positive feedbacks (which should generate low-density fitness penalties) in woody plant encroachment generally \citep{d2013vegetation} and specifically in this system \citep{d2010positive}. 
...
The very low transplant survival and recruitment rates that we measured also raises the issue of time scale. 
Previous studies suggest that creosote recruitment is strongly episodic, likely in response to large, infrequent monsoon precipitation events \citep{moreno2016seed,allen2008allometry}.
Similar patterns of episodic recruitment driven by large precipitation events have been observed in other cases of woody plant encroachment in aridlands \citep{harrington1991effects,weber2022woody}.
...
The connection between shrub recruitment and monsoon precipitation, combined with the observed and projected increase in the variability of monsoon precipitation in our study region \citep{petrie2014regional}, suggest that extending our deterministic model to accommodate inter-annual variability in climate and climate-dependent vital rates will be a critical next step. 
Because our wavespeed estimate is acutely sensitive to the seed-to-seedling transition, more so than any other demographic or dispersal process, we expect that a stochastic model incorporating many years of data may yield a faster predicted expansion speed driven by rare pulses of recruitment. 
...
However, our findings of pulled-wave dynamics and strong wavespeed sensitivity to seedling recruitment indicate that the present shrub ecotone is primed for expansion once the necessary climate conditions align, as they inevitably will in a more variable climate regime.

While not as strong a constraint as recruitment based on our sensitivity analysis, limited dispersal ability also contributed to the very slow speed of encroachment. 
...
For example, in aridlands in particular, overland flow of runoff may contribute to secondary seed movement following wind dispersal \citep{thompson2014secondary}. 
Interestingly, this form of movement would be most likely following large monsoon events, and therefore the same conditions that promote seedling recruitment may also promote long-distance dispersal, potentially amplifying a pulse of shrub encroachment. 
Biotic dispersal vectors may also play a role in secondary seed movement. The carpels of creosote bush do not offer rewards to animal disperers but seed predators, including ants and rodents, may contribute to seed movement particularly if some seeds can survive gut passage. 
Our wind dispersal model is a first step in understanding seed movement that makes our estimated dispersal kernels conservative. 
Integrating secondary dispersal vectors into a ``total'' dispersal kernel \citep{rogers2019total} will be a valuable direction for future research.
...
Our shrub encroachment model considered only one ``side'' of the grass-shrub ecotone, assuming that the population spreads into empty space. 
...
However, our transplant experiment provided no strong evidence for an influence of grass cover on seedling survival (Appendix XX).
...
While our current data do not allow us to quantify whether and how strongly resident grasses may slow down shrub encroachment, we can infer that competitive effects of grasses on shrubs are weaker than competitive effects of shrubs on shrubs (since shrub fitness increased from high to low shrub density). 
