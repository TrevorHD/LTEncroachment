...
Seeds were germinated on Pro-Mix potting soil (Quakertown, PA) in Fall 2014 and seedlings were transferred to 3.8 cm × 12.7 cm cylindrical containers and maintained in a greenhouse at Rice University.
...
To flexibly model density dependence, we used generalized additive models in the R package `mgcv' \citep{Wood2017}.
For each vital rate, we fit candidate models with or without a smooth term for local weighted density (among other possible covariates), which allowed for the possibility of non-monotonic responses to density if sufficiently supported by the data. 
To avoid over-fitting, we set the `gamma' argument of gam() to 1.4, which increases the complexity penalty, results in smoother fits \citep{Wood2017}, and therefore makes our approach more conservative.
