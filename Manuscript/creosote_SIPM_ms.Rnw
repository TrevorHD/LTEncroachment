...
Therefore, we re-fit the growth model with a skewed generalized $t$ (sgt) distribution, a five-parameter distribution on the real line that accommodates non-normal kurtosis and skew (R package `sgt': \citep{sgtpack}). 
