...
where $M(s,x)$ is the moment-generating function for the dispersal kernel associated with size $x$.
...
We then evaluated the empirical moment-generating function for each size $x$: $M(s)=\frac{1}{N} \sum_{i=1}^{N}e^{sr}$. 

Estimates of wavespeed were bootstrapped for a total of 1000 replicates.
...
Model selection for demographic vital rates was re-run for each bootstrap replicate, so this approach incorporates both parameter and model uncertainty. 
...
Thus, large increases in the size of new recruits were most likely to occur under low-density conditions. 
...
\paragraph{Recruitment and recruit size}
We observed \Sexpr{sum(CData$seedling_t1==1,na.rm=T)} natural recruitment events along our transects during the study years and our estimate recruitment rate, given total expected seed production in each window preceding the recruitment year, was very low ($2.47 \times 10^{-6}$, \ref{fig:vital_rates}F). 
...
As expected based on the vital rate results, the asymptotic population growth rate $\lambda$ declined monotonically with density (Fig. \ref{fig:lambda}). 
This was true across all bootstrap replicates, indicating high certainty that shrub fitness is maximized at zero density and thus that the expansion wave is ``pulled''. 
...
\newpage
\section*{Appendix B: Model selection results}
...
grow_aic<-xtable(aic_tables$grow_aic_out[,c("mean(size)","sd(size)","df","dAIC")], caption = "AIC model selection for mean and variance of future size", label = "tab:surv_aic",
                 hline.after=c(-1,0,1,2,3))
...
flower_aic<-xtable(aic_tables$flower_aic_out[,c("Pr(Flowering)","df","dAIC")], caption = "AIC model selection for flowering probability.", label = "tab:surv_aic",
                 hline.after=c(-1,0,1,2,3))
...
fruits_aic<-xtable(aic_tables$fruits_aic_out[,c("No. fruits","df","dAIC")], caption = "AIC model selection for fruit number.", label = "tab:surv_aic",
                 hline.after=c(-1,0,1,2,3))
...
recruit_aic<-xtable(aic_tables$recruit_aic_out[,c("Pr(Recruitment)","df","dAIC")], caption = "AIC model selection for recruitment probability.", label = "tab:surv_aic",
                 hline.after=c(-1,0,1,2,3))
...
recruitsize_aic<-xtable(aic_tables$recruitsize_aic_out[,c("mean(size)","sd(size)","df","dAIC")], caption = "AIC model selection for mean and variance of recruit size.", label = "tab:surv_aic",
                 hline.after=c(-1,0,1,2,3))
