...
\response{Because shrub cover was estimated every 10 meters, this is the minimum increment at which we could detect change in the shrub boundary. Therefore we are not confident that a rate of change from these data would be very meaningful, since we think the little change that occurred is consistent with measurement error (now mentioned in the new paragraph beginning line \ref{ms-broaden}).}
...
\response{Our study area has not experienced cattle grazing since 1973, and the last major fire occurred in 2001. Thus, the grazing and fire regimes were consistent throughout this study. This information has been added beginning at line \ref{ms-siteinfo}.}
