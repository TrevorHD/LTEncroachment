...
\newpage
\section*{Abstract}
\tom{The}{Much of this is re-written to address comments from last time. EM abstract word limit is 350, this is a little over.} encroachment of woody plants into grasslands is a global phenomenon with implications for biodiversity and ecosystem function. 
...
Expansion waves typically correspond to gradients of conspecific density -- \tom{high in the back and low at the front}{Brad asked: ``Any reason to not make these the same word (either "at" or "in")?'' I cannot tell you why, but the way I've written it here makes the most sense to my ear. Happy to change though if anyone objects.} -- and demographic rates may be sensitive to density due to intraspecific interactions like competition or facilitation.
...
We focused on wind dispersal of seeds, since little is known about the natural history of dispersal in this system and \tom{the seeds lack adaptations to attract frugivorous animals, such as bright coloration or fleshy fruit, though they may be moved by granivores}{Several of you objected to the original wording here, which said that ``seeds lack rewards to attract animal dispersers''. What I intended is that they do not have any adaptations characteristic of animal dispersal syndromes, but that does not exclude the possibility of being moved by granivores. My changes here may have made this sentence more awkward -- open to suggestions for how to best communicate what is known about seed dispersal.}.
\tom{Given the challenges of directly measuring seed dispersal, we instead built mechanistic dispersal kernels that predict seed movement based on properties of maternal plants, seeds, and wind; because it does not account for secondary biotic or abiotic dispersal vectors, this approach provided a conservative first step toward understanding seed movement.}{Added this in response to comments from Brad and Trevor, not sure if this is too apologetic or deflating.}
...
Given the population's size structure, we weighted the density of each window by the sizes of the plants, which we quantified as \tom{volume}{Scott suggested citing McAullife et al 2007 here - not sure why, that paper is about clonal rings.} (cm$^3$).
...
\tom{This assumption might lead us to over-estimate the recruitment rate, since existence of a seed bank would inflate the denominator of seedlings-per-seed.
However, a previous study at SNWR found relatively low densities of viable creosotebush seeds in soil, suggesting that this species does not form a persistent seed bank \citep{moreno2016seed}.}{This is new in response to your comments.}.
...
Therefore the dispersal kernel for a shrub of height $H$ was given \tom{by}{Trevor said he wanted to edit this equation, not sure if I have fixed what he wanted to edit.}:
...
\tom{We}{I've tried to clean up all the notation. Please give this section a close read.} used a spatial integral projection model to piece together seed dispersal and density-dependent demography, and generate predictions for the rate of shrub expansion that results from this combination of local and spatial processes. 
...
Corresponding to Eq. \ref{eq:Kdd} and assuming $\tilde{n}=0$, \tom{$H_{s}$}{Brad was confused by the H here, but $H_s$ is different from $H$. Are those sufficiently different to avoid confusion? I am trying to stay as consitent as possible with the related literature. $H$ is the standard notation for height in the dispersal literature, and $H_s$ is the standard notation in the SIPM literature.} is composed of
...
\paragraph{Growth}
Current size was strongly predictive of future size, as expected, and there was weak negative density dependence in mean future size conditioned on current size \tom{(Fig. \ref{fig:vital_rates}B)}{Scott asked: ``All of these lines look flat. Does that mean no growth?'' No, the flat lines mean there was no density dependence. Each line is a different size group. These results are hard to visualie because there is both size- and density -dependence. This also relates to some of Trevor's comments. This figure does in fact show future size conditional on current size, but I get why that does not jump out. Open to ideas for making any of this easier to digest.}. 
...
\paragraph{Recruitment and recruit size}
We observed \Sexpr{sum(CData$seedling_t1==1,na.rm=T)} natural recruitment events along our transects during the study years and our estimated recruitment rate, given total expected seed production in each window preceding the recruitment year, was very low ($2.47 \times 10^{-6}$, \ref{fig:vital_rates}E). 
...
The mean asymptotic speed was \Sexpr{round(mean(speed.dat$x),2)} m/year and the 5th--95th percentiles of the uncertainty distribution was \Sexpr{round(quantile(speed.dat$x,0.05),2)}--\Sexpr{round(quantile(speed.dat$x,0.95),2)} m/year (Fig. \ref{fig:wavespeed}A). 
...
In fact, to our knowledge, this is the slowest wavespeed estimate for a plant population in the \tom{demography-dispersal modeling literature (SIPMs and their matrix model progenitor}{Trevor said the wording is clunky here -- open to alternatives.}: \citep{neubert2000demography}). 
...
\tom{Creosotebush has a well documented history of expansion throughout the Southwest US, so it is clearly capable of rapid invasion.
Yet, whatever historical conditions allowed for shrub encroachment to its current extent, the encroachment wave at SNWR is presently stalled, under the conditions we observed it.}{Scott suggested we need to acknowledge that this species has a history of rapid invasion, so either we are missing something or the conditions under which we observed it are not the same conditions that allowed for rapid spread -- I am suggesting the latter here}. 
...
This was surprising given widespread evidence for positive feedbacks (which should generate low-density fitness penalties) in woody plant encroachment generally \citep{d2013vegetation} and specifically in our creosotebush bush system \citep{d2010positive}. 
...
\tom{Because}{Brad had a comment here about whether we may get different results with fat-tailed kernels. It's an interesting question but we are little constrained in our kernel options because the WALD approach is very tightly linked to the Inverse Gaussian. If we wanted to try other kernels we would need different data to get the necessary parameter estimates.} our wavespeed estimate is acutely sensitive to the seed-to-seedling transition, much more so than any other demographic or dispersal process, we expect that a stochastic model incorporating many years of data may yield a faster predicted expansion speed driven by rare pulses of recruitment \citep{ellner2012temporally}. 
...
Boyd and Brum's \citeyear{boyd1983postdispersal} study of creosotebush bush reproductive biology described ``contradiction in the literature about mode of dispersal'', citing evidence for a dominant role of wind but the additional possibility of seed movement by granivorous animals. 
...
While our current data do not allow us to quantify whether and how strongly resident grasses may slow down shrub encroachment, \tom{we can infer that competitive effects of grasses on shrubs are weaker than competitive effects of shrubs on shrubs.}{Scott asked: ``Do you really think there are no effect of an adult grass on a baby creosotebush seedling?'' No, I think there is an effect. What I was suggesting here is that the grass effect appears to be weaker than the shrub effect. Is that clear?} 
...
For a creosotebush bush, there is no better place to be than alone in a grassland, and that key result governs the spatial dynamics of this population. 
We found that wave of creosotebush bush expansion into Chihuahuan desert grassland is pulled by peak fitness at the leading edge.
...
Additionally, the WALD model assumes seed release from a single point source, which is not realistic for creosotebush bush; because seeds are released across the entire height of the shrub rather than from a point source, we integrated $p(r)$ across the uniform distribution from the grass canopy height to the shrub height.
...
surv_aic<-xtable(aic_tables$surv_aic_out[,c("Pr(Survival)","df","dAIC")], caption = "AIC model selection for survival probability.", label = "tab:surv_aic",
